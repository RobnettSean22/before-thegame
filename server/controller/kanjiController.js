module.exports = {
    addKanji : (req, res, next) => {
        ...
        db.add_kanji([index_number, user_id, folder_id ]).then((kanji) => res.status(200).send(kanji)).catch(err => {
    ...
    readKanji : (req,res, next) => {
        const db = req.app.get('db')
        const {user_id, folder_id} = req.params
        db.read_kanji([user_id, folder_id]).then((kanji) => res.status(200).send(kanji)).catch(err => {
